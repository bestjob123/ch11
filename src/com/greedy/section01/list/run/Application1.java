...
public class Application1 {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 1. List 인터페이스
		 *  -순서가 있는 데이터의 집합으로 테어터의 중복저장을 허용한다.
		 ...
		 *  -key를 set 방식으로 관리하기 때문에 대이터의 순서를 관리하지 않고 중복된 key를 허용하지 않는다.
		 *  단, value 는 중복된 값을 저장할 수 있다.
		 *  -HashMap, TreeMap, HashTable, Properties 등이 있다.
		 * 
		 * List 와 Set은 공통 부분을 Collection인터페이스에서 정의하고 있다.
		 * */
		
		/* ArrayList
		 * JDK 1.2 부터 제공되는 가장 많이 사용되는 컬렉션 클래스이다.
		 * 내부적으로 배열을 이용하여 요소를 관리하며 인덱스를 이용해 배열의 요소에 접근한다.
		 * ArrayList는 객체 배열의 단점을 보완하기 위해 만들어졌다.
		 * 배열은 크기를 변경할 수 없고 요소의 추가 ,삭제 정렬 등이 복잡하다느 단점을 가지고 있다.
		 * ArrayList는 크기 변경 요소의 추가 삭제 정렬 기능을 미리 구현해 놓고 제공해주고 있다.
...
//		Collection clist = new ArrayList();
		...
		/* 모든 컬렉션 프레임워크 크래스들은 제네릭 클래스로 작성되어 있다. */
...
//		stringList.add(123);
		 ...
		 * 1.8이하에서는 인터페이스에서 일반 메소드를 가질 수 없었기 떄문에,
		 ...
		 * 반복자 라고 불리우며ㅡ 반복문을 이용해서 목록을 하나씩 꺼내는 방식으로 사용하기 위함이다.
		 * 인덱스로 관리되는 컬렉션이 아닌 경우 반복문을 사용할 수 있는 방법이 존재하지 않기 때문에
		 * 요소에 하나씩 접근하기 위해 통일된 방식을 제공하는 것이다.
		 * 
		 * hasNext() : 다음 요소를 가지고 있는 경우 true 더이상 ㅏ요소가 없는 경우 false를 반환
		 * next() : 다음 요소를 반환
		 * */
